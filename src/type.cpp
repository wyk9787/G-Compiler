...
void type_error(std::string expression, std::string expect,
                std::string actual) {
  std::cerr << "In " << expression << std::endl;
  std::cerr << "Expecting type " << expect << ", receiving type " << actual
            << std::endl;
  exit(1);
}
...
std::string TInt::get_type() { return "Int"; }
...
std::string TFloat::get_type() { return "Float"; }
...
std::string TBool::get_type() { return "Boolean"; }
...
std::string TUnit::get_type() { return "unit"; }
...
bool TFunc::eq(const Typ &other) const {
  ...
  return res != nullptr &&
         *t1.get() == *const_cast<Typ &>(other).get_first_subtype().get() &&
         *t2.get() == *const_cast<Typ &>(other).get_second_subtype().get();
}
...
Shared_Typ TFunc::get_first_subtype() { return t1; }

Shared_Typ TFunc::get_second_subtype() { return t2; }
...
Shared_Typ TPair::get_first_subtype() { return t1; }

Shared_Typ TPair::get_second_subtype() { return t2; }
...
bool TList::eq(const Typ &other) const {
  ...
  return res != nullptr &&
         *t.get() == *const_cast<Typ &>(other).get_first_subtype().get();
}

std::string TList::get_type() { return "{" + t->get_type() + "}"; }

Shared_Typ TList::get_first_subtype() { return t; }
...
std::string TRef::get_type() { return "<" + t->get_type() + ">"; }

Shared_Typ TRef::get_first_subtype() { return t; }
